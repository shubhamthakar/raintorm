...
for i in $(seq -w 01 10); do
  ...
  ssh "$HOST" "if [ -d '$TARGET_DIR' ]; then
      ...
      cd '$TARGET_DIR' && sudo git config --global --add safe.directory /home/chaskar2/distributed-logger && sudo git remote set-url origin $REPO_URL && sudo git fetch origin && sudo git reset --hard origin/Rainstorm;
    else
